...
import PolymorphicRouter from "../VisualEngine/PolymorphicRouter";
import ProblemHeader from "../VisualEngine/ProblemHeader";

const LANG_MONACO = { python: "python", javascript: "javascript", cpp: "cpp", java: "java" };

const Val = ({ val }) => {
  if (Array.isArray(val) && val[0] === "REF") return <span className="text-[#6ab0f5] font-mono truncate">@{val[1]}</span>;
  if (Array.isArray(val) && ["FUNCTION", "JS_FUNCTION"].includes(val[0])) return <span className="text-purple-400 font-mono truncate">ƒ {val[1]}()</span>;
  if (typeof val === "string") return <span className="text-[#00b8a3] font-mono break-all">"{val}"</span>;
  if (val == null) return <span className="text-[#4d4d4d] font-mono">{val === null ? "null" : "—"}</span>;
  return <span className="text-[#ffb800] font-mono truncate">{String(val)}</span>;
};
  ...
  frames.length === 0 ? (<span className="text-[#4d4d4d] italic">Global scope — no frames</span>) : (
    <div className="space-y-3">
      {[...frames].reverse().map((frame, idx) => (
        <div key={idx} className="rounded-lg overflow-hidden border border-[#3d3d3d] bg-[#262626]">
          <div className={`px-3 py-1.5 text-[10px] font-bold tracking-widest uppercase flex items-center gap-2 border-b border-[#3d3d3d] ${idx === 0 ? "text-[#ffa116]" : "text-[#8d8d8d]"}`}>
            {idx === 0 && <span className="w-1.5 h-1.5 rounded-full bg-[#ffa116] flex-shrink-0" />}
            <span className="truncate">{frame.func_name}</span>
          </div>
          <div className="p-3 space-y-1.5 overflow-hidden">
            {frame.ordered_varnames?.length > 0 ? (frame.ordered_varnames.map((v) => (
                <div key={v} className="flex justify-between items-center py-0.5 gap-2 overflow-hidden">
                  <span className="text-pink-400 truncate flex-shrink-0 max-w-[40%]">{v}</span>
                  <div className="flex-1 text-right overflow-hidden"><Val val={frame.encoded_locals[v]} /></div>
                </div>
              ))
            ) : (<span className="text-[#4d4d4d] italic">empty frame</span>)}
          </div>
...
const HeapPanel = ({ heap = {} }) => {
  ...
  return entries.length === 0 ? (<span className="text-[#4d4d4d] italic">Heap is empty</span>) : (
    <div className="space-y-2">
      {entries.map(([id, value]) => (
        <div key={id} className="rounded-lg border border-[#3d3d3d] bg-[#262626] p-3 overflow-hidden">
          <div className="text-[#6ab0f5] text-[10px] font-bold mb-1.5">@{id}</div>
          <div className="text-[#8d8d8d] break-all text-[11px] overflow-auto max-h-32 custom-scrollbar">
...
const NavBtn = ({ onClick, children }) => (
  <button onClick={onClick} className="p-2 text-[#8d8d8d] hover:text-[#eff2f5] transition-colors rounded-lg hover:bg-white/5 flex items-center justify-center flex-shrink-0">
    {children}
  </button>
);
...
const PlaybackBar = ({ currentIndex, maxSteps, isPlaying, speed, setCurrentIndex, setIsPlaying, setSpeed }) => {
  const progress = maxSteps > 0 ? (currentIndex / maxSteps) * 100 : 0;
  return (
    <div className="flex-shrink-0 border-t border-[#3d3d3d] bg-[#282828] z-20 w-full flex flex-col">
      <div className="h-0.5 bg-[#3d3d3d] w-full">
        <div className="h-full bg-[#ffa116] transition-all duration-200" style={{ width: `${progress}%` }} />
      </div>
      
      {/* Responsive flex-wrap to prevent overlapping on small screens */}
      <div className="flex flex-wrap items-center justify-between px-4 py-2 md:h-14 gap-4 md:gap-2">
        
        {/* Controls */}
        <div className="flex items-center gap-1 flex-shrink-0">
          <NavBtn onClick={() => setCurrentIndex(0)}><MdFastRewind className="text-lg" /></NavBtn>
          <NavBtn onClick={() => setCurrentIndex((p) => Math.max(0, p - 1))}><MdSkipPrevious className="text-xl" /></NavBtn>
          <button onClick={() => setIsPlaying(!isPlaying)} className="mx-1 w-9 h-9 flex items-center justify-center bg-[#ffa116] hover:bg-[#ffb800] text-[#1a1a1a] rounded-full transition-all shadow-[0_0_14px_rgba(255,161,22,0.45)] flex-shrink-0">
            {isPlaying ? <MdPause className="text-lg" /> : <MdPlayArrow className="text-lg" />}
          </button>
          <NavBtn onClick={() => setCurrentIndex((p) => Math.min(maxSteps, p + 1))}><MdSkipNext className="text-xl" /></NavBtn>
          <NavBtn onClick={() => setCurrentIndex(maxSteps)}><MdFastForward className="text-lg" /></NavBtn>
        ...
        <div className="flex-1 flex items-center gap-3 min-w-[150px]">
          <span className="text-[10px] font-mono text-[#4d4d4d] w-8 text-right tabular-nums hidden sm:block">{currentIndex}</span>
          <input type="range" min="0" max={maxSteps} value={currentIndex} onChange={(e) => setCurrentIndex(Number(e.target.value))} className="flex-1 h-1 rounded-full cursor-pointer min-w-0" />
          <span className="text-[10px] font-mono text-[#4d4d4d] w-8 tabular-nums hidden sm:block">{maxSteps}</span>
        </div>
        
        {/* Speed Selector */}
        <div className="flex items-center gap-2 flex-shrink-0">
          <span className="text-[10px] font-bold uppercase tracking-widest text-[#4d4d4d] hidden sm:inline">Speed</span>
          <select value={speed} onChange={(e) => setSpeed(Number(e.target.value))} className="bg-[#262626] border border-[#3d3d3d] text-xs rounded-md px-2 py-1 outline-none cursor-pointer text-[#8d8d8d] hover:border-[#555] transition-colors">
            <option value={1000}>0.5×</option><option value={500}>1×</option><option value={250}>2×</option><option value={100}>4×</option>
          </select>
        </div>
...
const VisualPanel = ({
  ...
  setCurrentIndex, setIsPlaying, setSpeed,
  aiProps,
  vizSpec = null,  // ← NEW: visualization spec from AI analysis
}) => {
  const [activeTab, setActiveTab] = useState("output");
  ...
  return (
    <div className="flex flex-col flex-1 w-full bg-[#1a1a1a] text-white overflow-hidden min-h-0">
      ...
      {/* ── RESPONSIVE MAIN CONTAINER (Stacks vertically on small screens, side-by-side on large) ── */}
      <div className="flex flex-col lg:flex-row flex-1 min-h-0 w-full overflow-hidden">
        
        {/* ── LEFT HALF: Code + State (Takes 45% on desktop, 100% on mobile) ── */}
        <div className="flex flex-col sm:flex-row lg:flex-col xl:flex-row w-full lg:w-[45%] border-b lg:border-b-0 lg:border-r border-[#3d3d3d] min-w-0 flex-shrink-0">
          
          {/* Source Code Editor */}
          <div className="flex-1 min-w-0 flex flex-col h-64 sm:h-auto lg:h-1/2 xl:h-auto">
            <div className="flex items-center justify-between px-4 h-8 bg-[#282828] border-b border-[#3d3d3d] flex-shrink-0">
              <div className="flex items-center gap-2 overflow-hidden">
                <MdCode className="text-[#8d8d8d] text-xs flex-shrink-0" />
                <span className="text-[10px] tracking-widest uppercase text-[#8d8d8d] font-semibold truncate">Source</span>
              </div>
              <span className="text-[10px] font-mono text-[#ffa116] flex-shrink-0">{activeLine ? `line ${activeLine}` : "—"}</span>
            </div>
            <div className="flex-1 overflow-hidden relative">
              <Editor height="100%" language={LANG_MONACO[language]} value={userCode} onMount={handleEditorMount} options={{ readOnly: true, fontSize: 13, lineHeight: 21, fontFamily: "'JetBrains Mono', 'Fira Code', monospace", fontLigatures: true, minimap: { enabled: false }, scrollBeyondLastLine: false, padding: { top: 12, bottom: 12 }, renderLineHighlight: "none", glyphMargin: true, lineNumbers: "on", wordWrap: "off", folding: false, contextmenu: false, scrollbar: { vertical: "auto", horizontal: "auto" } }} theme="vs-dark" />
            </div>
          ...
          {/* State Sidebar */}
          <aside className="w-full sm:w-64 xl:w-72 flex flex-col border-t sm:border-t-0 sm:border-l lg:border-t lg:border-l-0 xl:border-t-0 xl:border-l border-[#3d3d3d] bg-[#222] flex-shrink-0 h-48 sm:h-auto lg:h-1/2 xl:h-auto">
            <div className="flex border-b border-[#3d3d3d] flex-shrink-0 overflow-x-auto custom-scrollbar">
              {TABS.map(({ id, label, icon: Icon }) => (
                <button key={id} onClick={() => setActiveTab(id)} className={`panel-tab flex items-center justify-center flex-1 min-w-[80px] gap-1.5 py-2.5 text-[10px] tracking-widest uppercase font-semibold transition-colors ${activeTab === id ? "text-[#eff2f5] border-b-2 border-[#ffa116]" : "text-[#8d8d8d] border-b-2 border-transparent"}`}>
                  <Icon className="text-xs flex-shrink-0" /><span className="truncate">{label}</span>
                </button>
              ))}
            </div>
            <div className="flex-1 overflow-y-auto p-4 font-mono text-xs text-[#eff2f5] custom-scrollbar">
              {activeTab === "output" && (<pre className="whitespace-pre-wrap text-[#8d8d8d] leading-relaxed break-words">{currentStep.stdout || (<span className="text-[#4d4d4d] italic">no output yet</span>)}</pre>)}
              {activeTab === "stack" && (<StackPanel frames={currentStep.stack_to_render} />)}
              {activeTab === "heap" && (<HeapPanel heap={currentStep.heap} />)}
            </div>
            <div className="border-t border-[#3d3d3d] px-4 py-2.5 flex items-center justify-between flex-shrink-0 bg-[#222]">
              <span className="text-[10px] font-mono text-[#4d4d4d] truncate">step {currentIndex + 1} / {maxSteps + 1}</span>
              {currentStep.event && (<span className="text-[10px] font-mono px-2 py-0.5 rounded bg-[#ffa116]/10 text-[#ffa116] border border-[#ffa116]/20 truncate max-w-[50%]">{currentStep.event}</span>)}
            </div>
          </aside>
        </div>

        {/* ── RIGHT HALF: Visualization Canvas (Takes 55% on desktop, scales dynamically) ── */}
        <div className="flex flex-col flex-1 min-w-0 bg-[#0f172a] relative z-0 h-80 lg:h-auto">
          <div className="flex items-center px-4 h-8 bg-[#282828] border-b border-[#3d3d3d] flex-shrink-0">
            <span className="text-[10px] tracking-widest uppercase text-[#8d8d8d] font-semibold truncate">Visualization Canvas</span>
          </div>

          <ProblemHeader vizSpec={vizSpec ?? currentStep?.vizSpec ?? null} />
          ...
          <div className="flex-1 overflow-hidden relative min-h-0">
            ...
            <PolymorphicRouter 
              ...
              aiFallbackEngine={aiProps?.primedEngine}
              aiVariables={aiProps?.analysis?.variables}
              vizSpec={vizSpec ?? currentStep?.vizSpec ?? null}
            />
          </div>
        </div>
      </div>
