...
const EngineRenderer = ({ structure, compact = false }) => {
  switch (structure.type) {
    case "RECURSION_TREE":
      return <RecursionTreeEngine data={structure.data} />
    case "TREE":
    ...
    case "DEQUE":
      return <DequeEngine data={structure.data} />;
    case "MATRIX":
    case "ARRAY":
    default:
      return <GridEngine data={structure.data} compact={compact} />;
  }
};

// --- Rank structures to pick the "Main Stage" ---
const getRank = (type) => {
...
export default function PolymorphicRouter({ currentFrame }) {
  ...
  // Sort structures by complexity rank to find the primary target
      ...
      <div className="flex-1 relative min-h-[300px]">
        <div className="absolute top-4 right-4 z-20 flex flex-col items-end gap-2 pointer-events-none">
          <div className="flex items-center gap-2">
            ...
            <span className="px-3 py-1 rounded-md bg-indigo-500/20 border border-indigo-500/40 text-xs font-mono text-indigo-200 shadow-sm">
              {primary.name}
            </span>
          </div>
        </div>
